...
int check_if_right(POINT argpt){
    ...
    if(argpt.y != -1){
        while(adjptr = adjPts(argpt , ptIdx++)){
            if(rsnMap[adjptr->y][adjptr->x] > 0 && rsnMap[adjptr->y][adjptr->x]< 9){
                tem = adjCnt(*adjptr , FLG , rsnMap);
                pem = adjCnt(*adjptr , UNK , rsnMap);
                if(rsnMap[adjptr->y][adjptr->x] < tem){
                    printf("%d %d is %d while F is %d\n" , adjptr->y , adjptr->x , rsnMap[adjptr->y][adjptr->x] , tem);
                    //print_map("rsnMap");

                    return -1;
                }
                else if(rsnMap[adjptr->y][adjptr->x] > tem && pem == 0){
                    printf("%d %d is %d while adjCntF is %d space is %d\n" , adjptr->y , adjptr->x , rsnMap[adjptr->y][adjptr->x] ,  tem , pem);
                    //print_map("rsnMap");
...
void print_map(char matrix_name[]) {
    int i, j;
                                            ...
                                            if(rsnMap[a][b] != 0){
                                                if(adjCnt(pt , FLG , rsnMap) == rsnMap[a][b] && adjCnt(pt , UNK , rsnMap) != 0){
                                                    ...
                                                    while(adjptr = adjPts(pt , ptIdx++)){
                                                        if(!rsnMap[adjptr->y][adjptr->x]){
                                                            rsnMap[adjptr->y][adjptr->x] = 10;
                                                        }
                                                        free(adjptr);
                                                    }
                                                }
    ...
    for(i = 0; i < MAPHI ; i++){
        for(j = 0; j < MAPWD; j++){
            ...
            if(map[i][j] != 0 && map[i][j] != 9 &&
                ...
                int ptIdx = 0;
                while(adjptr = adjPts(pt , ptIdx++)){
                    if(!map[adjptr->y][adjptr->x]){
                        if(arround_tem)
                            probability[adjptr->y][adjptr->x] += (1 - probability[adjptr->y][adjptr->x]) * arround_to_add;
                    }
                    free(adjptr);
                }

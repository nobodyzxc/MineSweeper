...
//	SendMessage(wndPane, WM_SETTEXT,  0, (LPARAM) "這是我個人使用的小算盤");
//	if(!(	wndPaneParent == GetActiveWindow()	)	)
...
//       printf("f %d s %d\n" , Arround(8 , 26 , "flag" , //array) , Arround(8 , 26 , "space" , array));
//        exit(0);
		analyze_screen();
		analyze_screen();
		if(1)
...
//		if(flags < 30 || All_count('d' , array) < 30){
...
//			if(i == 12 && j == 5) printf("flag arround %d arround space num %d self %d\n" , arround_flag_num , arround_space_num , array[i][j]);
...
/*			if(i == 4 && j == 0){
				printf("%d 4 0's tem\n" , tem_num);
				printf("s %d c %d f %d\n" , Arround( i , j ,"space" , array) , Arround(i , j , "clear" , array) , Arround(i , j , "flag" , array));
				exit(0);
			}*/
			if(tem_num < neighbor_num(i , j) && tem_num > 0)// **
			    border[i][j] = 1;
		}
...
*/ 
 //   printf("reasonging\n");
    ...
    while(point = rtnpt( y , x , which_pt++)){
     ...
     //   printf("%d %d -> %d %d" , y , x , point->y , point->x);
        free(point);
    //  Sleep(5000);
    }
...
//			printf("%d %d\n" , i , j);
			...
			while(1){
//				printf("%f %d\n" , dfs_count[i][j].num , dfs_count[i][j].exp);
//				Sleep(3000);
...
//	puts("              resuppose            !!!!!             !!!!!     \n");
	...
	print_array("dfs_count") ;//// hereherehereherehere
...
//					else if(left_mine_unknown == 1){
//						if(bestchosen > dfs_count[k][l]){// change here 因為有加權了 
//							bestY = k;
//							bestX = l;
//							bestchosen = dfs_count[k][l];
//						}						
...
//	    	open_window("minesweeper");
	    	...
	    	printf("num = %1.1f , exp = %d\n" , dfs_count[bestY][bestX].num , dfs_count[bestY][bestX].exp);
	    	printf("is ( %d )\n" , reason_array[bestY][bestX]);
	    	puts("Enter to continue...");
	//  	  getchar();
        	//Sleep(7000);
    		open_window("minesweeper");
        	Click(bestX , bestY  ,"left");
        }
	}
	else{
...
//	printf("%d -*- %d = %f 10^%d\n" , P , tem+1 , Pno.num , Pno.exp);

//	printf("%d -*- %d = %f 10^%d\n" , P-tem , 1 , Cno.num , Cno.exp);
...
/*
				tem_num = Arround( i , j ,"space" , reason_array) + Arround(i , j , "clear" , reason_array) + Arround(i , j , "flag" , reason_array);
...
//							printf("ERR1\n");
//							print_array(reason_array);
//							if(reason_array[15][8] == 9 && reason_array[14][8] == 10 && reason_array[15][16] == 9) puts("here") , Sleep(50000);
							
						}	
					}
					else{// fault
//						puts("ERR2");
//						printf("space %d flags = %d \n" , All_count(0 , reason_array) , dfs_flags);
//						print_array(reason_array);
//						if(reason_array[15][8] == 9 && reason_array[14][8] == 10 && reason_array[15][16] == 9) puts("here") , Sleep(50000);
					}
//					print_array(reason_array);
//					if(i == 0 && j == 0) printf("dfs_flags is %d \n" , dfs_flags) , Sleep(50000);
//					if(i == 4 && j ==0) Sleep(5000) , printf("FFFFF\n");
					if(dfs_flags >= 1){
						reason_array[i][j] = 9;			
						dfs_flags--;
//						print_array(reason_array);
...
//						if(reason_array[12][7] == 9 && reason_array[15][8] == 9){
//							printf("12 7 resu = %d\n\n" , result) ;
//							printf("%d\n" , check_if_right(i , j));
//							puts("");
//							print_array("reason_array");
...
//						if(history == 0){
							...
							printf("\nflags = %f should be zero while on %d %d stop\n\n",dfs_flags , i , j);
							print_array(reason_array);
//						}*/
					}
...
//	int add_combination;
//	unsigned long long float Czero , Cflag;
//	Czero = All_count(space , reason_array);
//	Cflag = dfs_flags;
//	puts("RRR");
	struct notation temC;
	temC = C(All_count(0 , reason_array) , (int)dfs_flags);
		...
		for(k = 0 ; k < Y ; k++)
			for(l = 0 ; l < X ; l++){
				if(dfs_count[k][l].num != -1)
					if(reason_array[k][l] == 9)
						addC( &dfs_count[k][l] , temC);
					if(neighbor_num(k , l) - Arround(k , l , "space" , array) - Arround(k , l , "flag" , array) == 0)
						addC( &dfs_count[k][l] , temC);
			}
		printf("----------flags left %f---------\n\n\n" , dfs_flags);
		left_mine_unknown = 1;
...
int need_flag(void){
	...
	//				printf("%d %d need flags %d - %d\n" , i , j,reason_array[i][j] , Arround(i , j , "flag" , reason_array) );
					return 1;
				}
			}
		}
...
//	printf("%d flags num is \n" , All_count(9));
	//flags = FLAGNUM - (float)All_count(9);

//	puts("will guess ... continue ? enter to continue");
//	getchar();
//	puts("will guess after 10 s");
//	Sleep(10000);
...
//							printf("all count 0 is %d \n", All_count(0));
//							printf("for_rand is %d\n" , for_rand);
//							printf("max = %f pro = %f , max change to %d %d\n" , max , probability[i][j], i , j);
							max = probability[i][j];
							y = i;
							x = j;
						}
